...
var fs = require("fs");
var talks = loadTalks(); //Object.create(null);
function loadTalks() {
  var result = Object.create(null), json;
  try {
    json = JSON.parse(fs.readFileSync("./node-talks.json", "utf8"));
  } catch (e) {
    json = {};
  }
...
function sendTalks(talks, response) {
  respondJSON(response, 200, {serverTime: Date.now(), talks: talks});
}
var waiting = [];
...
function registerChange(title) {
  ...
  fs.writeFile("./node-talks.json", JSON.stringify(talks));
}
